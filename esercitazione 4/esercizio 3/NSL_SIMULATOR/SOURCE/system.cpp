...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    for (int i=0; i<_npart; i++){
      ...
      _particle(i).setvelocity(1, 0.);
      _particle(i).setvelocity(2, 0.);
    }
    for (int i=0; i<_npart; i++){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        _bin_size_v = _v_max/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4
        _measure_pofv = true;
...
void System :: invert_time(){
  for(unsigned int j=0; j<_npart; j++){
    double xold=_particle(j).getposition(0, false);
    double yold=_particle(j).getposition(1, false);
    double zold=_particle(j).getposition(2, false);

    _particle(j).setpositold(0, _particle(j).getposition(0, true));
    _particle(j).setpositold(1, _particle(j).getposition(1, true));
    _particle(j).setpositold(2, _particle(j).getposition(2, true));

    _particle(j).setposition(0, xold);
    _particle(j).setposition(1, yold);
    _particle(j).setposition(2, zold);


  }
}

void System :: write_U_and_T(vec U, vec T, unsigned int nblock){
  ofstream coutf;
  coutf.open("../OUTPUT/UT.dat");
  if(coutf.is_open()){
    coutf << "# block"<< setw(12) <<"U" << setw(12) << "T" << endl;
    for(unsigned int i=0; i<U.size(); i++){
      coutf << nblock << setw(12) << U.at(i) << setw(12) << T.at(i) << endl;
    }
  } else cerr << "PROBLEM: Unable to open config.xyz" << endl;
  coutf.close();
  return;
