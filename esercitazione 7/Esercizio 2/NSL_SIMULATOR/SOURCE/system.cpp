  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        if (_sim_type==0){
          ofstream coutti("../OUTPUT/potential_energy_inst_MD.dat");
          coutti << "U_inst:" << endl;
          coutti.close();
        }
        if (_sim_type==1){
          ofstream coutti("../OUTPUT/potential_energy_inst_MC.dat");
          coutti << "U_inst:" << endl;
          coutti.close();
        }
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy){
    ...
    ofstream coutti;
    if (_sim_type==0){
      coutti.open("../OUTPUT/potential_energy_inst_MD.dat",ios::app);
      coutti << _measurement[_index_penergy] << endl;
      coutti.close();
    }
    if (_sim_type==1){
      coutti.open("../OUTPUT/potential_energy_inst_MC.dat",ios::app);
      coutti << _measurement[_index_penergy] << endl;
      coutti.close();
    }
  }
...
void System :: equilibrate(){
  ...
  if (_sim_type == 0){
    ...
    cout << endl << "Acceptance rate on equilibration steps: " << _naccepted/double(_eq_steps*_npart) << endl;
    cout << endl << "Resetting accepted moves counter..." << endl;
    _naccepted=0;
  }
  else{
