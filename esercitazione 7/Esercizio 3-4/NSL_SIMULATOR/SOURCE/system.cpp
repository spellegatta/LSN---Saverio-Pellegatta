...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    int spin_index=int(_rnd.Rannyu(0, double(_npart)));
    if (_sim_type == 3) {           // vero Gibbs per Ising 1D
      int s_left  = _particle[pbc(spin_index-1)].getspin();
      int s_right = _particle[pbc(spin_index+1)].getspin();
      double h    = _J*(s_left + s_right) + _H;
      double p_up = 1.0 / (1.0 + exp(-2.0 * _beta * h));
      if (_rnd.Rannyu() < p_up)
        _particle(spin_index).setspin(+1);
      else
        _particle(spin_index).setspin(-1);
      // (facoltativo) potresti tenere un contatore di "mosse" ma non serve acceptance
    }
  } else {           // M(RT)^2
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> _eq_steps;
      coutf << "EQUILIBRATION STEPS= " << _eq_steps << endl;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = (8*M_PI*_rho/3.)*(1./(3*pow(_r_cut, 9))-1./pow(_r_cut,3)); //FIXED IN EXERCISE 7
        ...
        coutt.close();
        if (_sim_type==0){
          ofstream coutti("../OUTPUT/total_energy_inst_MD.dat");
          coutti << "U_inst:" << endl;
          coutti.close();
        }
        if (_sim_type==1){
          ofstream coutti("../OUTPUT/total_energy_inst_MC.dat");
          coutti << "U_inst:" << endl;
          coutti.close();
        }
        _nprop++;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        dr = sqrt( dot(distance,distance) );
        if(dr<_halfside.min()){
          unsigned int i = int(dr/_bin_size);
          _measurement[i]+=2;
        }
        if(dr < _r_cut){
  ...
  if (_measure_pofv){
    double v_1part, p_max, p; 
    double cum_sum=0; //somma cumulativa
    int index;
    vec distribution_v(_n_bins_v);
    distribution_v.zeros();
    for(unsigned int j=0; j<_npart; j++){
      v_1part=norm(_particle.at(j).getvelocity());
      //cout << "v:" << v_1part;
      index=static_cast<int>(v_1part/_bin_size_v);
      //cout << " index: " << index << endl;
      if (index<_n_bins_v)
      _measurement(_index_pofv+index)++;
      else{
        _n_part_excluded++;
      }
      //cout << _npart << "   " << j << "   " << size(_measurement) << endl;
    }
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
      ...
      for (int i=0; i<_npart; i++){
        ...
        //cout << _J << setw(12) << s_i << setw(12) << s_j << setw(12) << _H << endl;
      ...
      ofstream coutti;
      if (_sim_type==0){
        coutti.open("../OUTPUT/total_energy_inst_MD.dat",ios::app);
        coutti << _measurement[_index_tenergy] << endl;
        coutti.close();
      }
      if (_sim_type==1){
        coutti.open("../OUTPUT/total_energy_inst_MC.dat",ios::app);
        coutti << _measurement[_index_tenergy] << endl;
        coutti.close();
      }
  }
  ...
  if (_measure_magnet){
    ...
    for (unsigned int i=0; i<_npart; i++){
      spin_sum+=_particle(i).getspin();
    }

    spin_sum = double(spin_sum) / double(_npart);
    _measurement[_index_magnet]=spin_sum;
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv) {
    double E2 = pow(tenergy_temp*_npart,2);
    _measurement[_index_E2] = E2;   // indice E2
  }
  ...
  if(_measure_chi){
    ...
    spin_sum = pow(double(spin_sum),2) / double(_npart);
    _measurement[_index_chi]=spin_sum;
...
void System :: averages(int blk){
  ...
  if (_measure_tenergy){
    ...
    _U.push_back(sum_average/double(blk));
    _U.push_back(this->error(sum_average, sum_ave2, blk));
  }
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
  ...
  if (_measure_pressure){
    ...
    coutf << setw(12) << blk
          ...
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
    _P.push_back(sum_average/double(blk));
    _P.push_back(this->error(sum_average, sum_ave2, blk));
  }
  // GOFR //////////////////////////////////////////////////////////////////////
    if (_measure_gofr){
    ...
    if (!coutf.is_open()) {
    cerr << "Errore: impossibile aprire il file gofr.dat" << std::endl;
    return;
    }
    else{
      for (unsigned int i=0; i<_n_bins; i++){
        double r_center=_bin_size*i+0.5*_bin_size;
        double dV=(4*M_PI/3.)*(pow(_bin_size*(i+1), 3)-pow(_bin_size*i, 3));
        double norm=1./(_rho*_npart*dV);
  ...
  if (_measure_pofv){
    ...
    double norm=1./(_bin_size_v*(_npart-_n_part_excluded));
    for (unsigned int i=0; i<_n_bins_v; i++){
      double v_center=_bin_size_v*i+0.5*_bin_size_v;
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
    if (_measure_magnet){
      ...
      coutf << setw(12) << blk
            ...
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf.close();
      _M.push_back(sum_average/double(blk));
      _M.push_back(this->error(sum_average, sum_ave2, blk));
    }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv) {
    double var=_average(_index_E2)-_average(_index_tenergy)*_average(_index_tenergy)*_npart*_npart;

    _average(_index_cv)=_beta*_beta*var/double(_npart);
    ...
    coutf << setw(12) << blk << setw(12) << _average(_index_cv) << setw(12) <<  _global_av(_index_cv)/double(blk) << setw(12) << this->error(_global_av(_index_cv), _global_av2(_index_cv), blk) << endl;
    _cv.push_back(_global_av(_index_cv)/double(blk));
    _cv.push_back(this->error(_global_av(_index_cv), _global_av2(_index_cv), blk));
  }
    ...
    if (_measure_chi){
      ...
      coutf << setw(12) << blk
            << setw(12) << _beta*sum_average/double(blk)
            << setw(12) << _beta * error(_global_av[_index_chi], _global_av2[_index_chi], blk );

      coutf.close();
      _chi.push_back(_beta*sum_average/double(blk));
      _chi.push_back(_beta * error(_global_av[_index_chi], _global_av2[_index_chi], blk ));
    }  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
...
void System :: equilibrate(){
  if (_sim_type == 1){
    cout << endl << "Resetting accepted moves counter..." << endl;
    _naccepted=0;
    _nattempts=0;

  }
  for(int j=0; j < get_equilibration_steps()-1000; j++){ //loop over steps in a block
    step();
  }

  if (_sim_type == 0){
    assert (_measure_temp and _measure_kenergy);
    double T_counter=0;
    for(int j=0; j < 1000; j++){ //loop over steps in a block
      step();
      measure();
      T_counter+=_measurement[_index_temp];
    }
    cout << endl << "Mean T* on the last 1000 steps = " << T_counter/1000. << endl;
    //non voglio registrare le misure appena fatte
    ofstream coutti("../OUTPUT/total_energy_inst_MC.dat");
    coutti << "U_inst:" << endl;
    coutti.close();
    ...
    cout << endl << "Acceptance rate on equilibration steps: " << _naccepted/double(_nattempts) << endl;
    cout << endl << "Resetting accepted moves counter..." << endl;
    _naccepted=0;
    _nattempts=0;
  }
  else{
    cerr << endl << "Equilibration not carried out: simulation type not valid (only 0 and 1 are valid values)" << endl;
  }
}
...
double System :: get_cv(){
  return _cv.at(0);
}
double System :: get_chi(){
  return _chi.at(0);
}
double System :: get_M(){
  return _M.at(0);
...
double System :: get_cv_error(){
  return _cv.at(1);
}

double System :: get_chi_error(){
  return _chi.at(1);
}
double System :: get_M_error(){
  return _M.at(1);
}
double System :: get_U_error(){
  return _U.at(1);
}
double System :: get_P_error(){
  return _P.at(1);
}
unsigned int System :: get_equilibration_steps(){
  return _eq_steps;
}
